...
import { terminateRental, deleteUnit } from "@/app/actions/rental-actions"
...
export function UnitCustomersGrid() {
  ...
  const handleUnlinkRental = async (rentalId, unitId) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Update local state - update the unit to show as available
  ...
  const filteredUnits = sortedUnits.filter(unit => {
    ...
    if (activeTab === "available") return unit.status === "available"
    if (activeTab === "occupied") return unit.status === "occupied"
    if (activeTab === "maintenance") return unit.status === "maintenance"
    return true
  })
                ...
                {customer ? (
                  <div className="bg-blue-50 border border-blue-200 rounded-md p-2 mt-2">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <button
                        onClick={() => setRentalToUnlink({ id: activeRental.id, unitId: unit.id, unitNumber: unit.unit_number, customerName: `${customer.first_name} ${customer.last_name}` })}
